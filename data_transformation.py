...
def transform_times_data(unavailability_data):
    ...
    latest_date = datetime(9999, 12, 31, 23, 59, 59)  # Use a far future date as the latest date
    ...
    for id, dates in unavailability_data:
        try:
            if dates is None or not isinstance(dates, list):
                print(f"Skipping id {id}: dates is None or not a list")
            ...
            for start, end in dates:
                try:
                    ...
                    transformed_dates.append((datetime_to_timestamp(start), datetime_to_timestamp(end)))
                except Exception as e:
                    print(f"Error processing start-end pair (start: {start}, end: {end}) for id {id}: {e}")
